...
def write_log(msg):
    if logstatus == ("on"):
        with open(myfile, "a") as log:
            ...
            log.write(
                datetime.now().strftime("%Y/%d/%m, %H:%M:%S.%f") + ": " + msg + "\n"
            )
...
try:
    ...
    if config.plugins.xtraEvent.timerMod.value == "Clock":
        ...
        def startDownload():
            logout(
                data="----------------------------------------------------------------------------------------------- start download 10 sec"
            )
            from . import download

            download.downloads("").save()
            logout(
                data="----------------------------------------------------------------------------------------------- ende download 10 sec "
            )
            startTimer()
...
def Plugins(**kwargs):
    return [
        PluginDescriptor(
            name="speedy_xtraEvent {}".format(xtra.version),
